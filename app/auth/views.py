...
def confirm(token):
    ...
    Эта функция обрабатывает процесс подтверждения email пользователя по токену, который был отправлен на
    почту при регистрации. Пользователь может подтвердить свой email, перейдя по ссылке, содержащей токен.
